...
import com.minigod.security.protocol.cubp.vo.request.CubpUserInfoReqVo;
import com.minigod.security.protocol.cubp.vo.response.CubpSecurityUserInfoResVo;
...
import com.minigod.security.protocol.enums.CustomOpenAccountEnum.*;
import com.minigod.security.protocol.enums.OcrEnum;
...
public class OpenAccountServiceImpl extends BaseBeanFactory implements OpenAccountService {
    ...
    private boolean verifyOpenInfoData(OpenAccountAppointmentPojo pojo) {
        return true;
    }
    ...
    public void saveOrUpdateOpenInfo(Integer userId, OpenInfoReqParams params) {
        ...
        OpenAccountAppointmentPojo pojo = JSONObject.parseObject(JSONObject.toJSONString(info), OpenAccountAppointmentPojo.class);

        Integer openAccessWay = pojo.getOpenAccountAccessWay();

        String email = pojo.getEmail();
        String idCard = pojo.getIdNo();
        String bankCard = pojo.getBankNo();
    ...
    public OpenUserInfoResVo getOpenProgress(Integer userId, OpenProgressReqParams params) {
        ...
        // 查询cubp用户中心开户数据
        CubpUserInfoReqVo cubpUserInforeqVo = new CubpUserInfoReqVo();
        cubpUserInforeqVo.setUserId(userId);
        CubpSecurityUserInfoResVo cubpSecurityUserInfoResVo = restCubpHelper.selectSecuritiesUserInfo(cubpUserInforeqVo);

        // 查询到cubp用户数据
        if (cubpSecurityUserInfoResVo != null) {
            // clientStatus 客户状态[0-正常 1-冻结 2-挂失 3-销户 D-休眠 E-不合格 F-锁定]
            String clientStatus = cubpSecurityUserInfoResVo.getClientStatus();
            ...
            openType = cubpSecurityUserInfoResVo.getOpenAccountType();
            accountType = cubpSecurityUserInfoResVo.getFundAccountType();
            fundAccount = cubpSecurityUserInfoResVo.getFundAccount();
            tradeAccount = cubpSecurityUserInfoResVo.getTradeAccount();
