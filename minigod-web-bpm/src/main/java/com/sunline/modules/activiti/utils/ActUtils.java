package com.sunline.modules.activiti.utils;
...
public class ActUtils {
    ...
    private static ProcessEngine processEngine = (ProcessEngine) SpringContextUtils.getBean("processEngine");
    ...
    public static void turnTransition(String taskId, String activityId, Map<String, Object> variables, String comment) throws Exception {
        ...
        try {
            ...
            logger.error("taskService.saveTask(task) is error", e);
        }
...
//        dealMultiSequential(task.getProcessInstanceId(), comment, task.getTaskDefinitionKey());
     ...
     * @param userId 被转办人id
     */
    public static void transferAssignee(String taskId, String userId) {
        try {
            taskService.setAssignee(taskId, userId);
        } catch (Exception e) {
            logger.error("taskService.setAssignee(taskId, userId) is error", e);
        }
    }

    public static FlowNode getCurrentFlowNode(String instanceId) {
//        String instanceId = "170001";
...
//    public static FlowNode getFlowNodeById(String instanceId){
//        HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery().processInstanceId(instanceId).singleResult();
////        String instanceId = "170001";
////        Task task = taskService.createTaskQuery().processInstanceId(instanceId).singleResult();
//        BpmnModel bpmnModel = processEngine.getRepositoryService().getBpmnModel(nodeId);
//        FlowNode flowNode = (FlowNode) bpmnModel.getFlowElement(task.getTaskDefinitionKey());
//
//        return flowNode;
//    }
     ...
     * @return
     */
    public static boolean isFlowEnd(@NotNull(message = "实例ID不能为空") String processInstanceId) {
        ...
        return null != processInstance.getEndTime();
    }
     ...
     * @return
     */
    public boolean isFlowEnd2(@NotNull(message = "实例ID不能为空") String processInstanceId) {
        HistoricActivityInstance lastActivityInstance = getLastActivityInstance(processInstanceId);

        return "endEvent".equalsIgnoreCase(lastActivityInstance.getActivityType());
    }
    ...
    public HistoricActivityInstance getLastActivityInstance(String processInstanceId) {
        List<HistoricActivityInstance> historicActivityInstances = getHistoryActivityInstance(processInstanceId);
        if (null != historicActivityInstances && !historicActivityInstances.isEmpty()) {
            HistoricActivityInstance currentActivityInstance = historicActivityInstances.get(historicActivityInstances.size() - 1);
     ...
     * @return
     */
    public List<HistoricActivityInstance> getHistoryActivityInstance(String processInstanceId) {
        List<HistoricActivityInstance> historicActivityInstances = processEngine.getHistoryService().createHistoricActivityInstanceQuery().processInstanceId(processInstanceId).orderByHistoricActivityInstanceStartTime().asc().list();

        return historicActivityInstances;
    }

    public static ProcessDefinition getlastProcessDefinition(String processDefinitionKey) {
     ...
     * @param taskId
     */
    public static void endProcess(String taskId, String comment) throws Exception {
