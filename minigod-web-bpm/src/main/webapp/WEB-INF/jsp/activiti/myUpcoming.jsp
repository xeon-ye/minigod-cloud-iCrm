                    ...
                    <tr name="task_${task.taskId }">
                        ...
                        <td>
                            <button class="layui-btn layui-btn-small" type="button"
                                    onclick="doTaskTab('${task.actKey}','${task.busId}','${task.instanceId}','${task.taskId}','${task.defId}','${task.nodeType}','${task.dealId}','${task.dealName}')">
...
<script>
     ...
     * 任务办理Tab
     */
    function doTaskTab(actKey, busId, instanceId, taskId, defid, nodeType, dealId, dealName) {
        var url = "${webRoot}/act/deal/flowInfoTab?flag=2&actKey=" + actKey + "&busId=" + busId + "&instanceId=" + instanceId
            + "&taskId=" + taskId + "&defId=" + defid + "&nodeType=" + nodeType + "&dealId=" + dealId + "&dealName=" + dealName
        //弹框层
        layer.open({
            scrollbar: false,
            type: 2,
