...
<div class="main-container" id="main-container">
    <div class="" style="margin-top: 10px;">
        <form class="layui-form" id="search-from" method="post"
              action="${webRoot}/fundWithdrawRefund/checkAuditList">
            ...
            <div class="layui-form-item" style="padding: 10px 50px;">
                ...
                <shiro:hasPermission name="fundWithdrawRefund:expDealList">
            ...
            <table id="table-list" class="layui-table" lay-size="">
                ...
                <c:forEach items="${page.result}" var="info" varStatus="i">
                    <tr name="${info.id }">
                        ...
                        <td>${info.bankNo}</td>
                        <td>
                                ${fns:getCodeName("FUND_WITHDRAW_REFUND_STATUS", info.applicationStatus)}
                        </td>
                        <td>
                            <div class=" btn-group ">
                                ...
                                <c:if test="${info.assignDrafter == currentUserId}">
                                    <button class="layui-btn layui-btn-small" type="button"
                                            onclick="doTaskTab('fundWithdrawRefundApplication','${info.applicationId}','${info.instanceId}','','${info.defid}',''
                                                    ,'${info.assignDrafter}','${info.currentNode}','${fns:getCodeName("FUND_BANK_TYPE",info.withdrawType)}'
                                                    ,'${fns:getCodeName("FUND_WITHDRAW_REFUND_STATUS",info.applicationStatus)}','${info.bankName}')">
...
<script>
    ...
    function expAuditList() {
        ...
        var obj = $('#search-from').serialize();
//        layer.alert(obj);
        window.location.href = '${webRoot}/fundWithdrawRefund/expDealList?fundWithdrawRefundApplyEntity=&' + obj;
...
//        setTimeout(refresh, 6000);
    ...
    function applyTask(applicationIds, currentUserId, createTime) {
        ...
        confirm("您确定申领此业务吗?", function () {
            ...
            var url = "${webRoot}/fundWithdrawRefund/batchApplyTaskHandle";
    ...
    function deliverApplyTask() {
        ...
        confirm("您确定释放此业务吗?", function () {
            ...
            var url = "${webRoot}/fundWithdrawRefund/deliverApplyTask";
            $.post(url, params, function callback(result) {
                if (result.code == '0') {
                    toast(result, function () {
                        // 刷新列表
