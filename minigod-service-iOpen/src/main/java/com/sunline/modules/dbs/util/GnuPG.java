 ...
 */
public class GnuPG {
    private static final Logger logger = LoggerFactory.getLogger(GnuPG.class);
    // Konstants:
    private final String kGnuPGCommand = "gpg";
     ...
     * @return true upon success
     */
    public boolean encrypt(String luser, String keyID, String filepath, String inStr) {
        boolean success;

        if (StringUtils.isEmpty(luser)) {
            success = runGnuPG("-r " + keyID + " -as --encrypt --output " + filepath, inStr);
        } else {
            success = runGnuPG("--local-user " + luser + " -r " + keyID + " -as --encrypt --output " + filepath, inStr);
        }
        if (success && this.gpg_exitCode != 0)
            success = false;
        return success;
...
//		String		fullCommand = commandArgs;
        ...
        try {
            ...
            logger.info("io Error" + io.getMessage());
            return false;
        }
        ...
        if (inputStr != null) {
            ...
            try {
                ...
                logger.info("Exception at write! " + io.getMessage());
                return false;
            }
        }
        ...
        try {
            ...
            logger.info("Exception at waitfor! " + i.getMessage());
            return false;
        }
    ...
    private boolean createTempFile(String inStr) {
        ...
        try {
            ...
            logger.info("Cannot create temp file " + e.getMessage());
            return false;
        }
        ...
        try {
            ...
            logger.info("Cannot write temp file " + e.getMessage());
            return false;
        }
...
//			pgp.decrypt (args[1], args[2]);
        ...
        pgp.encrypt("JiuFu-DBS-PRO", "dbs3yV3-20200228", "C:\\Users\\JOY\\Desktop\\test\\DSG_VAHKL.20190701_1450.CSV.pgp C:\\Users\\JOY\\Desktop\\test\\account.csv", null);
        logger.info("result: " + pgp.gpg_result + "\n\n");
        logger.info("error: " + pgp.gpg_err + "\n\n");
        logger.info("exit: " + pgp.gpg_exitCode + "\n\n");
