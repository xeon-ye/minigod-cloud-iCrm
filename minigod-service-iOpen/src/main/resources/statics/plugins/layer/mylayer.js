 ...
 */
function myToast(option,index,cancelCallback){
    if(typeof option == 'string'){
        $layer.msg(option,{icon: index, shade:0.6, time: 1500 }, function(){
            if(typeof cancelCallback=='function'){
                cancelCallback();
            }
        });
    }else{
        var ops = $.extend({icon: index, shade:0.6, time: 1500 },option);
