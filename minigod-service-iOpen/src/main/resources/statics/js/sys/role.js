var searchUrl = location.search; //获取url中"?“符后的字串
if (searchUrl.indexOf("?") != -1) {
    var roleType = decodeURIComponent(searchUrl.substr(1).replace("roleType=",""));
}
$(function () {
    $("#jqGrid").jqGrid({
        url: '../sys/role/list',
        datatype: "json",
        postData:{
            'roleType': roleType
        },
        colModel: [
            {label: '角色名称', name: 'name', index: "name", width: 75},
            {
                label: '角色类型', name: 'roleType', width: 80,sortable:false, formatter: function (value, options, row) {
                return value == 0 ?
                    '<span class="label label-success">内部角色</span>' :
                    '<span class="label label-danger">外部角色</span>';
            }
            },
            {
                label: '状态', name: 'status', width: 80,sortable:false, formatter: function (value, options, row) {
                return value == 0 ?
                    '<span class="label label-success">正常</span>' :
                    '<span class="label label-danger">禁用</span>';
            }
            },
            {label: '创建人', name: 'userName', index: "createName", width: 75},
            {label: '备注', name: 'remark', width: 100},
            {
                label: '创建时间',
                name: 'createTime',
                index: "create_time",
                width: 80,
                sortable:false,
                formatter: function (value) {
                    var dateObj = new Date(value);

                    var year = dateObj.getFullYear();//年
                    var month = dateObj.getMonth() + 1;//月  (注意：月份+1)
                    var date = dateObj.getDate();//日
                    var day = dateObj.getDay();
                    var weeks = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"];
                    var week = weeks[day];//根据day值，获取星期数组中的星期数。
                    var hours = dateObj.getHours();//小时
                    var minutes = dateObj.getMinutes();//分钟
                    var seconds = dateObj.getSeconds();//秒

                    if (month < 10) {
                        month = "0" + month;
                    }
                    if (date < 10) {
                        date = "0" + date;
                    }
                    if (hours < 10) {
                        hours = "0" + hours;
                    }
                    if (minutes < 10) {
                        minutes = "0" + minutes;
                    }
                    if (seconds < 10) {
                        seconds = "0" + seconds;
                    }

                    var newDate = year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds;
                    return newDate;
                }
            },
            {
                label: '更新时间',
                name: 'updateTime',
                index: "updateTime",
                width: 80,
                formatter: function (value) {
                    var dateObj = new Date(value);

                    var year = dateObj.getFullYear();//年
                    var month = dateObj.getMonth() + 1;//月  (注意：月份+1)
                    var date = dateObj.getDate();//日
                    var day = dateObj.getDay();
                    var weeks = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"];
                    var week = weeks[day];//根据day值，获取星期数组中的星期数。
                    var hours = dateObj.getHours();//小时
                    var minutes = dateObj.getMinutes();//分钟
                    var seconds = dateObj.getSeconds();//秒

                    if (month < 10) {
                        month = "0" + month;
                    }
                    if (date < 10) {
                        date = "0" + date;
                    }
                    if (hours < 10) {
                        hours = "0" + hours;
                    }
                    if (minutes < 10) {
                        minutes = "0" + minutes;
                    }
                    if (seconds < 10) {
                        seconds = "0" + seconds;
                    }

                    var newDate = year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds;
                    return newDate;
                }
            }
        ],
        viewrecords: true,
        height: 385,
        rowNum: 10,
        rowList: [10, 30, 50],
        rownumbers: true,
        rownumWidth: 25,
        autowidth: true,
        multiselect: true,
        pager: "#jqGridPager",
        jsonReader: {
            root: "page.list",
            page: "page.currPage",
            total: "page.totalPage",
            records: "page.totalCount"
        },
        prmNames: {
            page: "page",
            rows: "limit",
            order: "order"
        },
        gridComplete: function () {
            //隐藏grid底部滚动条
            $("#jqGrid").closest(".ui-jqgrid-bdiv").css({"overflow-x": "hidden"});
        }
    });
});


var setting = {
    data: {
        simpleData: {
            enable: true,
            idKey: "id",
            pIdKey: "parentId",
            rootPId: -1
        },
        key: {
            url: "nourl"
        }
    },
    check: {
        enable: true,
        nocheckInherit: true
    }
};

var setting2 = {
    data: {
        simpleData: {
            enable: true,
            idKey: "channelId",
            pIdKey: "parentId",
            rootPId: -1
        },
        key: {
            url: "nourl"
        }
    },
    check: {
        //复选框
        enable: true,
        nocheckInherit: true,
        chkboxType:  { "Y": "s", "N": "s" }
    }
};

var ztree;
var ztree2;
var ztree3;
var option;
var vm = new Vue({
    el: '#rrapp',
    data: {
        q: {
            name: null
        },
        showList: true,
        title: null,
        role: {}
    },
    methods: {
        query: function () {
            vm.reload();
        },
        add: function () {
            vm.showList = false;
            vm.title = "新增";
            vm.role = {};
            option=0;
            vm.getMenuTree(null);
            vm.role.roleType = roleType;
        },
        update: function () {
            var id = getSelectedRow();
            if (id == null) {
                return;
            }
            option = -1;
            vm.showList = false;
            vm.title = "修改";
            vm.getRole(id);
        },
        del: function (event) {
            var ids = getSelectedRows();
            if (ids == null) {
                return;
            }

            confirm('确定要禁用选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: "../sys/role/delete",
                    data: JSON.stringify(ids),
                    success: function (r) {
                        if (r.code == 0) {
                            toast(r.msg, function () {
                                vm.reload();
                            });
                        } else {
                            alertMsg(r.msg);
                        }
                    }
                });
            });
        },
        enabled: function (event) {
            var ids = getSelectedRows();
            if (ids == null) {
                return;
            }

            confirm('确定要启用选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: "../sys/role/enabled",
                    data: JSON.stringify(ids),
                    success: function (r) {
                        if (r.code == 0) {
                            toast(r.msg, function () {
                                vm.reload();
                            });
                        } else {
                            alertMsg(r.msg);
                        }
                    }
                });
            });
        },
        menu: function () {
            var id = getSelectedRow();
            if (id == null) {
                return;
            }
            option = 1;
            vm.showList = false;
            vm.title = "菜单授权";
            vm.getMenu(id);
        },
        organ: function () {
            var id = getSelectedRow();
            if (id == null) {
                return;
            }
            option = 2;
            vm.showList = false;
            vm.title = "数据授权";
            vm.getOrgan(id);
        },
        channel: function () {
            var id = getSelectedRow();
            if (id == null) {
                return;
            }
            option = 3;
            vm.showList = false;
            vm.title = "渠道授权";
            vm.getChannel(id);
        },
        getRole: function (id) {
            $.get("../sys/role/info/" + id, function (r) {
                vm.role = r.role;
                if(option>0){
                var menuIds = vm.role.menuIdList;
                //勾选角色所拥有的菜单
                for (var i = 0; i < menuIds.length; i++) {
                    var node1 = ztree.getNodeByParam("id", menuIds[i]);
                    ztree.checkNode(node1, true, false);
                }

                var organIds = vm.role.organIdList;
                //勾选角色所在的组织机构
                for (var i = 0; i < organIds.length; i++) {
                    var node2 = ztree2.getNodeByParam("id", organIds[i]);
                    ztree2.checkNode(node2, true, false);
                }

                var channelIds = vm.role.channelIdList;
                //勾选角色所在的渠道
                for (var i = 0; i < channelIds.length; i++) {
                    var node3 = ztree3.getNodeByParam("channelId", channelIds[i]);
                    ztree3.checkNode(node3, true, false);
                }
                }
            });
        },
        saveOrUpdate: function (event) {
            vm.role.option = option;
            vm.role.channelIdList=new Array();
            vm.role.menuIdList=new Array();
            vm.role.organIdList=new Array();
            //获取选择的菜单
            if(option ==1 || option==0){
                var nodes = ztree.getCheckedNodes(true);
                var menuIdList = new Array();
                for (var i = 0; i < nodes.length; i++) {
                    menuIdList.push(nodes[i].id);
                }
                vm.role.menuIdList = menuIdList;
            }
            //获取选择的组织
            if(option ==2 || option==0) {
                var organNodes = ztree2.getChangeCheckedNodes(true);
                var organIdList = new Array();
                for (var i = 0; i < organNodes.length; i++) {
                    var status = organNodes[i].getCheckStatus();
                    if (status.half) {
                        continue;
                    }
                    organIdList.push(organNodes[i].id);
                }
                vm.role.organIdList = organIdList;
            }
            //获取选择的渠道
            if(option == 3 || option==0) {
                var nodes = ztree3.getCheckedNodes(true);
                var channelIdList = new Array();
                for (var i = 0; i < nodes.length; i++) {
                    channelIdList.push(nodes[i].channelId);
                }
                vm.role.channelIdList = channelIdList;
            }
            if (vm.role.name == null) {
                layer.alert("角色名称必填，不可为空！");
            } else if (vm.role.roleType == null) {
                layer.alert("角色类型必选，不可为空！");
            } else if (vm.role.status == null) {
                layer.alert("状态必选，不可为空！");
            } else {
                var url = vm.role.id == null ? "../sys/role/save" : "../sys/role/update";
                $.ajax({
                    type: "POST",
                    url: url,
                    data: JSON.stringify(vm.role),
                    success: function (r) {
                        if (r.code == 0) {
                            if (vm.role.id == null) {
                                alert(r, function (index, layer) {
                                    vm.reload();
                                }, function (index) {
                                    vm.role.name = '';
                                    vm.role.remark = '';
                                })
                            } else {
                                toast(r.msg, function () {
                                    vm.reload();
                                });
                            }

                        } else {
                            alertMsg(r.msg);
                        }
                    }
                });
            }
        },
        getMenu: function (id) {
            $.ajaxSetup({
                async: true
            });

            try {
                //加载菜单树
                $.get("../sys/menu/perms", function (r) {
                    ztree = $.fn.zTree.init($("#menuTree"), setting, r.menuList);
                    //展开所有节点
                    //ztree.expandAll(true);
                    var nodes = ztree.getNodes();
                    if (nodes.length > 0) {
                        for (var i = 0; i < nodes.length; i++) {
                            ztree.expandNode(nodes[i], true, false, false);
                        }
                    }

                    $.get("../sys/role/info/" + id, function (r) {
                        vm.role = r.role;
                        var menuIds = vm.role.menuIdList;
                        //勾选角色所拥有的菜单
                        for (var i = 0; i < menuIds.length; i++) {
                            var node1 = ztree.getNodeByParam("id", menuIds[i]);
                            ztree.checkNode(node1, true, false);
                        }
                    });
                });

            } catch (err) {
                layer.alert('error=' + err);
            }
        },
        getOrgan: function (id) {
            $.ajaxSetup({
                async: true
            });

            try {
                //加载资源树
                $.get("../sys/organ/organTree", function (r) {
                    ztree2 = $.fn.zTree.init($("#organTree"), setting, r.organTree);
                    //展开所有节点
                    //ztree.expandAll(true);
                    $.get("../sys/role/info/" + id, function (r) {
                        vm.role = r.role;
                        var organIds = vm.role.organIdList;
                        //勾选角色所在的组织机构
                        for (var i = 0; i < organIds.length; i++) {
                            var node2 = ztree2.getNodeByParam("id", organIds[i]);
                            ztree2.checkNode(node2, true, false);
                        }
                    });
                });

            } catch (err) {
                layer.alert('error=' + err);
            }

        },
        getChannel: function (id) {
            $.ajaxSetup({
                async: true
            });

            try {
                //加载资源树
                $.get("../userChannelInfo/perms", function (r) {
                    ztree3 = $.fn.zTree.init($("#channelTree"), setting2, r.userChannelList);
                    var nodes = ztree3.getNodes();
                    if (nodes.length > 0) {
                        for (var i = 0; i < nodes.length; i++) {
                            ztree3.expandNode(nodes[i], true, false, false);
                        }
                    }

                    $.get("../sys/role/info/" + id, function (r) {
                        vm.role = r.role;
                        var channelIds = vm.role.channelIdList;
                        //勾选角色所在的渠道
                        for (var i = 0; i < channelIds.length; i++) {
                            var node3 = ztree3.getNodeByParam("channelId", channelIds[i]);
                            ztree3.checkNode(node3, true, false);
                        }
                    });
                });
            } catch (err) {
                layer.alert('error=' + err);
            }
        },
        getMenuTree: function (id) {

            $.ajaxSetup({
                async: true
            });

            try {
                //加载菜单树
                $.get("../sys/menu/perms", function (r) {
                    ztree = $.fn.zTree.init($("#menuTree"), setting, r.menuList);
                    //展开所有节点
                    //ztree.expandAll(true);
                    var nodes = ztree.getNodes();
                    if (nodes.length > 0) {
                        for (var i = 0; i < nodes.length; i++) {
                            ztree.expandNode(nodes[i], true, false, false);
                        }
                    }
                });

                //加载资源树
                $.get("../userChannelInfo/perms", function (r) {
                    ztree3 = $.fn.zTree.init($("#channelTree"), setting2, r.userChannelList);
                    var nodes = ztree3.getNodes();
                    if (nodes.length > 0) {
                        for (var i = 0; i < nodes.length; i++) {
                            ztree3.expandNode(nodes[i], true, false, false);
                        }
                    }
                });

                //加载资源树
                $.get("../sys/organ/organTree", function (r) {
                    ztree2 = $.fn.zTree.init($("#organTree"), setting, r.organTree);
                    //展开所有节点
                    //ztree.expandAll(true);
                });
            } catch (err) {
                layer.alert('error=' + err);
            }

            if (id != null) {
                vm.getRole(id);
            }
        },
        reload: function (event) {
            vm.showList = true;
            // var page = $("#jqGrid").jqGrid('getGridParam', 'page');
            $("#jqGrid").jqGrid('setGridParam', {
                postData: {
                    'name': $("#roleName").val(),
                    'status': $("#status").val()
                },
                page: 1
            }).trigger("reloadGrid");
        }
    }
});

